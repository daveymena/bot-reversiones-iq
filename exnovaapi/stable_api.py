...
class Exnova:
    ...
    def connect(self, sms_code=None):
        ...
        self.api = ExnovaAPI(
            "ws.trade.exnova.com", self.email, self.password)
        check = None
    ...
    def update_ACTIVES_OPCODE(self):
        try:
            ...
            logging.error(f'**error** Failed to update ACTIVES OPCODE: {e}')
            # Si falla, intentar reconectar y reintentar una vez
            try:
                logging.warning('**warning** Attempting to reconnect and retry update_ACTIVES_OPCODE')
                self.connect()
                time.sleep(2)
                self.get_ALL_Binary_ACTIVES_OPCODE()
                self.instruments_input_all_in_ACTIVES()
                ...
                logging.error(f'**error** Retry also failed: {retry_error}')
    ...
    def get_ALL_Binary_ACTIVES_OPCODE(self):
        ...
        if init_info is None:
            logging.error('**error** Failed to get binary actives opcode - init_info is None')
            return
            
        try:
            for dirr in (["binary", "turbo"]):
                if dirr in init_info["result"] and "actives" in init_info["result"][dirr]:
                    for i in init_info["result"][dirr]["actives"]:
                        OP_code.ACTIVES[(init_info["result"][dirr]
                                         ["actives"][i]["name"]).split(".")[1]] = int(i)
        except Exception as e:
            logging.error(f'**error** Failed to process binary actives opcode: {e}')
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if not self.check_connect():
                    logging.warning('**warning** WebSocket connection lost, reconnecting...')
                    self.connect()
                    time.sleep(2)
                ...
                while True:
                    ...
                    try:
                        ...
                        pass
                    time.sleep(0.1)  # Pequeña pausa para evitar CPU overload
                
                try:
                    if self.api.api_option_init_all_result and self.api.api_option_init_all_result.get("isSuccessful") == True:
                        return self.api.api_option_init_all_result
                except:
                    pass
                    
            except Exception as e:
                logging.error(f'**error** get_all_init exception: {e}')
                # Si hay una excepción específica de WebSocket cerrado, reconectar
                if "Connection is already closed" in str(e):
                    logging.warning('**warning** WebSocket closed, reconnecting...')
                    self.connect()
                    time.sleep(2)
            
            retry_count += 1
            if retry_count < max_retries:
                logging.warning(f'**warning** Retrying get_all_init ({retry_count}/{max_retries})')
                time.sleep(3)
        
        logging.error('**error** get_all_init failed after multiple retries')
        return None
    ...
    def __get_digital_open(self):
        # for digital options
        digital_data_result = self.get_digital_underlying_list_data()
        if digital_data_result is None:
            logging.error('**error** Failed to get digital underlying list data')
            return
...
# __________________for Digital___________________
    ...
    # thank thiagottjv
    # https://github.com/Lu-Yi-Hsun/exnovaAPI/issues/65#issuecomment-513998357
